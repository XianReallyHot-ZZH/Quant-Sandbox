...
class RiskManager(Protocol):
    """成交前的风控闸门:引擎接受任何意图之前先问它;五条内置规则在
    下一课(#5)落地。Protocol = 结构化鸭子类型——不用继承,只要 check
    方法签名对得上,任何对象都能当 RiskManager 用(测试里的 stub 正是
    这么做的)。"""
...
class BacktestEngine:
    ...
    risk_manager: RiskManager | None = None  # 规则在 #5 落地
    # init=False:这三个字段不出现在构造参数里,由引擎自己维护;名字前
    # 的下划线表示「内部状态,外部只读快照」。
    ...
    def run(self, candles: list[Candle], symbol: str, timeframe: str) -> BacktestResult:
        ...
        # run 开头清空三份运行期账目:同一实例可以重复 run,不跨运行串味。
        self._pending = []
